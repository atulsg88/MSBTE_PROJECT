		...
		<div class="col-lg-12 m-b30">
			...
			<form class="edit-profile m-b30" action="ExamForm?action=saveExam" id="myForm" method="post">
				...
				<div class="form-group row">
						...
						<input type="hidden" name="student_photo" value="<%=(editStudent!=null)?editStudent.getStud_photo():""%>">
						<%} %>
			...
			<table class="table table-bordered table-striped mb-0" id="datatable-editable">
				...
				<tbody id="subject">
					...
					<button name="button" type="button" class="btn" id="Submit">Save</button>
				</div>
				</div>
			</form>
			 </div>
	          ...
	          url:"ExamForm?action=getFormDetails&course_name="+course_name+"&semester="+semester,
	       	   success:function(result){
	       		$.each(result, function(i, subject) {
	       		 ...
	       		 var myarray = sub_choice.split(',');
       			 $('<tr>').html("<td style=font-size:15px;>" + result[i].subject + "</td> <td><input type='checkbox' id='th"+(i+1)+"' value='TH' "+(myarray.includes('TH')?'checked="checked"': "TH")+" disabled> </td> <td><input type='checkbox' id='tw"+(i+1)+"' value='TW' "+(myarray.includes('TW')?'checked="checked"': "")+" disabled></td> <td><input type='checkbox' id='pr"+(i+1)+"' value='PR' "+(myarray.includes('PR')?'checked="checked"': "")+" disabled></td> <td><input type='checkbox' id='or"+(i+1)+"' value='OR' "+(myarray.includes('OR')?'checked="checked"': "")+" disabled></td> <td><input type='checkbox' id='sw"+(i+1)+"' value='SW' "+(myarray.includes('SW')?'checked="checked"': "")+" disabled></td> <td><input type='checkbox' id='tm"+(i+1)+"' value='TM' "+(myarray.includes('TM')?'checked="checked"': "")+" disabled></td> <td><input type='checkbox' id='pm"+(i+1)+"' value='PM' "+(myarray.includes('PM')?'checked="checked"': "")+" disabled></td>").appendTo('#subject');
	       		});
	       	   } 
	          });
           }
...
$("#Submit").click(function()
{
	...
	var pr="";
     $("#subject").find('tr').each(function(i,el){
		...
		var th= $("input[type='checkbox'][id='th"+(i+1)+"']:checked").val();
		if(th==undefined){th=""}
		var tw= $("input[type='checkbox'][id='tw"+(i+1)+"']:checked").val();
		if(tw==undefined){tw=""}
	    pr= $("input[type='checkbox'][id='pr"+(i+1)+"']:checked").val(); 
		if(pr==undefined){pr=""}
		var or= $("input[type='checkbox'][id='or"+(i+1)+"']:checked").val();
		if(or==undefined){or=""}
		var sw= $("input[type='checkbox'][id='sw"+(i+1)+"']:checked").val();
		if(sw==undefined){sw=""}
		var tm= $("input[type='checkbox'][id='tm"+(i+1)+"']:checked").val();
		if(tm==undefined){tm=""}
		var pm= $("input[type='checkbox'][id='pm"+(i+1)+"']:checked").val();
		if(pm==undefined){pm=""}
